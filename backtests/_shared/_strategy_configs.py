...
_PRIMARY_INSTRUMENT_SENTINELS = {
    None,
    "__PRIMARY_INSTRUMENT__",
    "__PRIMARY_INSTRUMENTS__",
}
        ...
        importable_configs.append(
            ImportableStrategyConfig(
                ...
                config=_normalized_config(
                    raw_config=raw_config, instrument_id=instrument_id
                ),
            )
        )
...
def resolve_strategy_factory(
    *,
    strategy_factory,
    strategy_configs: Sequence[StrategyConfigSpec] | None,
):
    ...
    def _factory(instrument_id: InstrumentId) -> Strategy:
        ...
        if len(strategies) != 1:
            raise ValueError(
                "Prediction-market single-market runners currently support exactly one "
                "strategy config per run."
            )
        return strategies[0]
