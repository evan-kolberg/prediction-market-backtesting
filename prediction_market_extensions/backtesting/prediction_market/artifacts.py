...
class PredictionMarketArtifactBuilder:
    ...
    def build_result(
        ...
        *,
        ...
        instrument_positions = self._filter_report_rows(positions_report, instrument_id=instrument_id)
    ...
    def build_market_artifacts(
        ...
        return {
            loaded_sim.market_id: self._build_market_artifacts_for_loaded_sim(
                ...
                fills_report=self._filter_report_rows(fills_report, instrument_id=str(loaded_sim.instrument.id)),
    ...
    def build_joint_portfolio_artifacts(
        ...
        for loaded_sim in loaded_sims:
            price_points = extract_price_points(
                loaded_sim.records, price_attr="mid_price" if self.data_type == "quote_tick" else "price"
            )
        ...
        dense_equity_series, dense_cash_series = (
            prediction_market_research._dense_account_series_from_engine_for_markets(
                engine=engine, market_prices=market_prices_by_market_id, initial_cash=self.initial_cash
            )
        )
        pnl_series = (
        ...
        return {
            "joint_portfolio_pnl_series": prediction_market_research._series_to_iso_pairs(pnl_series)
            if not pnl_series.empty
            else [],
            "joint_portfolio_equity_series": prediction_market_research._series_to_iso_pairs(dense_equity_series)
            if not dense_equity_series.empty
            else [],
            "joint_portfolio_cash_series": prediction_market_research._series_to_iso_pairs(dense_cash_series)
    ...
    def _build_market_artifacts_for_loaded_sim(
        ...
        *,
        ...
        user_probabilities, market_probabilities, outcomes = build_brier_inputs(
            price_points, window=self.probability_window, realized_outcome=loaded_sim.realized_outcome
        )
        ...
        if self.emit_html or self.return_chart_layout:
            ...
            try:
                ...
                if self.emit_html:
                    artifacts["chart_path"] = save_legacy_backtest_layout(chart_layout, str(output_path), chart_title)
    ...
    def resolve_chart_output_path(self, *, market_id: str) -> Path:
        ...
        if "{" in raw_path:
            try:
                ...
                raise ValueError("chart_output_path may only reference {name} and {market_id}.") from exc
            path = Path(resolved)
            if not path.suffix:
        ...
        if path.suffix:
            if self.sim_count == 1:
                return resolve_repo_relative_path(path)
            return resolve_repo_relative_path(path.with_name(f"{path.stem}_{market_id}{path.suffix}"))
    ...
    def _build_market_summary_series(
        ...
        *,
        ...
        legacy_fills = prediction_market_research.legacy_plot_adapter._convert_fills(fills_report, legacy_models)
        ...
        series_artifacts: dict[str, Any] = {
            ...
            "user_probability_series": prediction_market_research._series_to_iso_pairs(user_probabilities)
            if not user_probabilities.empty
            else [],
            "market_probability_series": prediction_market_research._series_to_iso_pairs(market_probabilities)
            if not market_probabilities.empty
            else [],
            "outcome_series": prediction_market_research._series_to_iso_pairs(outcomes) if not outcomes.empty else [],
            "fill_events": prediction_market_research._serialize_fill_events(
        ...
        dense_equity_series, dense_cash_series = prediction_market_research._dense_account_series_from_engine(
            engine=engine, market_id=loaded_sim.market_id, market_prices=market_prices, initial_cash=self.initial_cash
        )
        pnl_series = (
            dense_equity_series - float(dense_equity_series.iloc[0])
            if not dense_equity_series.empty
        ...
        series_artifacts["pnl_series"] = (
            prediction_market_research._series_to_iso_pairs(pnl_series) if not pnl_series.empty else []
        )
        ...
        series_artifacts["cash_series"] = (
            prediction_market_research._series_to_iso_pairs(dense_cash_series) if not dense_cash_series.empty else []
        )
        return series_artifacts
