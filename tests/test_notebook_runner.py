...
def test_load_notebook_metadata_prefers_explicit_notebook_metadata(
    ...
    metadata = notebook_runner.load_notebook_metadata(
        notebook_path,
...
def test_load_notebook_metadata_falls_back_to_markdown_heading(
    tmp_path: Path,
) -> None:
    notebook_path = tmp_path / "backtests" / "demo_runner.ipynb"
    notebook_path.parent.mkdir(parents=True)
...
def test_execute_notebook_runner_replaces_auto_embed_cell_with_latest_html(
    tmp_path: Path,
) -> None:
    notebook_path = tmp_path / "backtests" / "demo_runner.ipynb"
    notebook_path.parent.mkdir(parents=True)
            ...
            nbformat.v4.new_code_cell(
                "\n".join(
                    [
                        ...
                        "(output_root / 'demo_multi_market.html').write_text('<html><body>summary</body></html>', encoding='utf-8')",
    ...
    notebook_runner.execute_notebook_runner(
        notebook_path,
        project_root=tmp_path,
    )
    ...
    assert "demo_multi_market.html" in source
    assert "../output/demo_multi_market.html" in source
