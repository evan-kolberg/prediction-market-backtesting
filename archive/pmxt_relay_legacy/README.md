...
> Preferred path: mirror raw PMXT archive hours and process them locally.
> Set `PMXT_RELAY_MIRROR_ONLY=1` to keep discovery, mirroring, and `/v1/raw/*`
> online while disabling server-side filtered processing and filtered-hour API
> routes.
>
> The older ClickHouse-backed filtered API path is now legacy. It still exists
> in code for existing deployments, but local processing is the intended path
> going forward.

The relay keeps two separate roles available:

- raw mirror: discover upstream PMXT archive hours, mirror them under `raw/`,
  and optionally expose `/v1/raw/*`
- local processing: process a local raw mirror into the existing filtered
  market-hour parquet layout with `uv run python -m pmxt_relay process-local`

What it does:

- mirrors every hourly raw PMXT Polymarket archive parquet file
- keeps polling the PMXT archive index for new hours
- can expose those mirrored raw files through `/v1/raw/*`
- can process the same raw mirror locally into filtered market-hour parquet
  files that match the loader's existing cache format
- still has a legacy server-side filtered store seam for existing deployments,
  but that path is no longer the preferred architecture
...
That means the backtests do not need a new schema. Local processing writes the
same filtered shape the loader already consumes today.

## Pipeline Stages

Each hourly archive file moves through four stages on the legacy server-side
processing path:
...
When `PMXT_RELAY_MIRROR_ONLY=1`, mirrored hours are left in a disabled
processing state instead of being queued for server-side filtering.
...
This means the relay keeps two persistent layers in mirror-only mode:

- `raw/` stores the mirrored PMXT archive hours as-is
- `state/` stores the relay SQLite metadata/index

Local processing can write filtered output to any directory you choose via
`uv run python -m pmxt_relay process-local --filtered-root ...`.

When the legacy server-side filtered path is enabled, parsed filtered rows live
either under `processed/` and `filtered/` or in ClickHouse, depending on
`PMXT_RELAY_FILTERED_STORE_BACKEND`.

Temporary `.tmp` files only exist during atomic writes.

## Processes

For the preferred raw mirror path, run:

- Worker: `uv run python -m pmxt_relay worker`
- optional raw API: `uv run python -m pmxt_relay api`
...
To process mirrored raw data locally into filtered market-hour parquet:
...
The worker handles discovery and raw mirroring:
...
4. keeps polling for new hours

Server-side filtered processing is now legacy. If you still need it, keep
`PMXT_RELAY_MIRROR_ONLY=0` and follow the existing filtered-store sections
below.

## Fresh Box Expectations

On a fresh mirror-only box, the steady-state layout should look like this:

- `raw/` grows with mirrored PMXT hourly parquet files
- `state/relay.sqlite3` tracks archive, queue, and event metadata
- `/v1/raw/*` can serve mirrored parquet if you keep the API or reverse proxy
- `processed/` and `filtered/` do not need to exist on the server
